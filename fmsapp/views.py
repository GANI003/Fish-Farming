...
from .models import Dam, Department, Staff, Fish, TimeLine, Sales, Debtors, Order
from django.contrib.auth.views import LoginView, TemplateView
from django.views.generic import CreateView, ListView, DetailView, UpdateView, View
from .forms import LoginForm, DamForm, StaffForm, DebtorsForm, DepartmentForm, RegisterForm, FishForm, HarvestForm, \
    SalesForm, OrderForm
...
class Dashboard1(LoginRequiredMixin, TemplateView):
    template_name = 'fmsapp/dashboard/index1.html'
    login_url = 'fms:login1'
...
class Login1(LoginView):
    template_name = 'fmsapp/home/login1.html'
    form_class = LoginForm
    model = get_user_model()
    ...
    def get_success_url(self):
        ...
        self.request.session['username'] = query.username
        # self.request.session['']
        url = resolve_url('fmsapp:dashboard1')
        return url
...
class Logout1(View):
    def get(self, request):
        logout(request)
        return redirect('fmsapp:login1', permanent=True)
...
def register(request, password=None):
    ...
    if form.is_valid():
        ...
        user.save()
        subject = 'Hello'
        message = 'Hi {username}, thank you for registering in Fish Farmimg App.'
    ...
    return render(request,'fmsapp/home/signup.html',context)
...
class DamList1(LoginRequiredMixin, ListView):
    model = Dam
    login_url = 'fms:login1'
    template_name = 'fmsapp/dam/index1.html'
    context_object_name = 'damlist'
...
class SalesAll1(LoginRequiredMixin, ListView):
    login_url = 'fms:login1'
    model = Sales
    template_name = 'fmsapp/sales/index1.html'
    context_object_name = 'sales'
...
# order
def order(TemplateView):
    LoginRequiredMixin, CreateView
    return redirect('https://pages.razorpay.com/pl_KnRhJU0LJFtjeh/view/')